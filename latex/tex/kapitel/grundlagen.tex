...
oder auch eigenschaftsbasierter Identifikatoren können von allen Parteien deterministisch aus einer Software abgeleitet werden.
\citeauthor{CISA2023} sieht als das Kriterium für diese Art, dass sich die Identifikatoren ausschließlich aufgrund von den Dateien der Software berechnet werden kann, wie etwa SHA1 oder SHA256 mit Standards wie OmniBOR\footnote{\url{https://omnibor.io}}.
Hashes haben die Herausforderungen, dass nicht immer klar ist, welche Datei(en) der Generierungsfunktion gegeben werden sollen, und, dass diese bekannt und öffentlich verwendbar sein muss.

Hashes von Dateien und Projekten werden jedoch selten von den bei der \metaeffektsp eingesetzten \acrshortpl{vdb} zur Produktidentifikation verwendet, darum haben diese wenig Relevanz.
In dieser Arbeit wird zusätzlich auch \acrshort{purl} als inherente Identifikator gesehen, da in den Software-Inventaren der \metaeffektsp recht eindeutig \acrshortpl{purl} abgeleitet werden können.

\paragraph{Autoritative Identifikatoren} % definierte, extrinsische
