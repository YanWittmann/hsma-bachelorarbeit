...
Die \textit{Correlation Utilities} sind ein spezialisiertes Experten-Werkzeug zur Analyse von Software-Inventaren im Rahmen der Erstellung und Pflege von Korrelationsdaten.
...
\begin{itemize}
    \item Wenn das Artefakt \enquote{Derived CPE URIs} hat, die nicht von einem der weiteren \acrshort{cpe}-Attribute (Additional, Ignores, \ldots) abgedeckt sind, dann müssen diese auf ihre Richtigkeit geprüft und je nach Entscheidung zu einem der manuellen Attribute hinzugefügt werden.
    Diese Art von \acrshort{cpe} auf einem Artefakt nennt sich im Tool \enquote{Untouched CPEs}, da sie auf diesem Artefakt noch nicht behandelt wurden.
    ...
    Entweder wird ein vorhandener Korrelationseintrag ergänzt, oder ein neuer angelegt.
    ...
    \item Wenn das Artefakt nur manuelle \acrshort{cpe}-Attribute gesetzt hat, dann handelt es sich bei diesem Artefakt um ein bereits bekanntes welches bereits bearbeitet wurde.
    In den meisten Fällen ist hier keine neue Arbeit nötig, jedoch gibt es manchmal Fälle, in denen ein Artefakt in der Zwischenzeit eine neue \acrshort{cpe} zugewiesen bekommen hat, etwas bei dem vorherigen Durchgang übersehen wurde, oder es sich um eine Fehlidentifikation in den Korrelationsdaten durch einen zu breit aufgespannten Matching-Raum handelt und diese fehlerhaft hinzugefügt wurde.
    Je nach Attribut-Kombination sind unterschiedliche Fälle wahrscheinlicher, hierfür bekommt man ein Gefühl über die Zeit in der man mit dem Tool arbeitet.
\end{itemize}
