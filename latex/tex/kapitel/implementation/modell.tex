...
Bisher festgelegte Referenztypen sind \enquote{homepage}, \enquote{repository}, \enquote{...}.
Um jedoch die Erweiterbarkeit zu sichern, kann als Schlüssel jeder Wert verwendet werden. % TODO: fill types
...
\subsection{Matching von Artefakt-Repräsentationen}\label{subsec:model-matching}
...
Zunächst wird die Artefakt-Selektion auf einzelnen Knoten eingeführt, dann werden mehrere Knoten über Vererbungsregeln kombiniert und deren Regeln erläutert.

\subsubsection{Artefakt-Selektor}
...
\subsubsection{Vererbung von Selektoren}

Wie in \autoref{subsubsec:model-edges-relations} angerissen haben Artefakt-Knoten die Möglichkeit

\subsubsection{Ergebnisse der Selektion}

Das Ergebnis des Selektionsprozesses eine Menge an \texttt{artifact}-Knoten im Graphen, die dem eingehenden Software-Artefakt entsprechen.
Diese gefundenen Knoten dienen als Ausgangspunkte für die anschließende Durchquerung des Graphen (\autoref{subsec:model-traversal}), um die zugehörigen Repräsentationen zu ermitteln.
